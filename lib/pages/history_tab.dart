...
class _HistoryTabState extends State<HistoryTab> {
  ...
  Widget build(BuildContext context) {
    ...
    final pendingCount = history.where((item) => !item.isSynced).length;
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              syncedCount > 0
                  ? '$syncedCount item berhasil disinkron!'
                  : 'Tidak ada item untuk disinkron.',
            ),
          ),
        );
        setState(() {});
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error retry sync: $e')));
      }
    } finally {
      if (mounted) {
